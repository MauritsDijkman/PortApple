...
namespace TiledMapParser
{
	...
	public class Polygon {
		...
		GXPEngine.Core.Vector2[] vecs=null;

		public int GetNumPoints() {
			if (vecs==null) { Initialize(); }
			return vecs.Length;
		}
		public GXPEngine.Core.Vector2 GetPoint(int index) {
			if (vecs==null) { Initialize(); }
			if (index<0 || index>=vecs.Length) { return new GXPEngine.Core.Vector2(0, 0); }
			return vecs[index];
		}

		void Initialize() {
			string[] pts = points.Split(' ');
			vecs=new GXPEngine.Core.Vector2[pts.Length];
			for (int i=0;i<vecs.Length;i++) {
				string[] coords = pts[i].Split(',');
				vecs[i]=new GXPEngine.Core.Vector2(float.Parse(coords[0]), float.Parse(coords[1]));
			}
		}
	}
